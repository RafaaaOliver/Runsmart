            ...
            function reparcelar(){ // função de salvar no suitelet
                try{
                    ...
                    for (i in listaDeJuros){
                        listaDeJuros[i] = Date.parse(listaDeJuros[i])
                        console.log(listaDeJuros[i])
                    }
                    function getMaxOfArray(numArray) {
                        return Math.max.apply(null, numArray);
                    }
                    console.log('tipo de dado: ', typeof listaDeJuros[0])
                    var maiorvigencia = getMaxOfArray(listaDeJuros)
                    maiorvigencia = new Date(maiorvigencia)
                    var dataFinal = dia + '/' + mes + '/' + ano
                    console.log('data em número: ', maiorvigencia)
