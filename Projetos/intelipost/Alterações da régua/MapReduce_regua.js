...
define(["require", "exports", "N/search", "N/email", "N/render", "N/runtime", "N/log", "N/https", "N/record"], function (require, exports, search_1, email_1, render_1, runtime_1, log_1, https_1, record_1) {
    ...
    var map = function (ctx) {
        ...
        try {
            ...
            }, feriadosList = feriados(), EMAIL_PREVENTIVO = EmailPreventivo(dtVencimentoGv, invoiceRec.getValue({ fieldId: 'total' }), dadosEmail), EMAIL_REATIVO = EmailReativo(dtVencimentoGv, Number(invoiceRec.getValue({ fieldId: 'total' })), dadosEmail);
            if (!data_prorrogada) {
            ...
            default:
                ...
                if (Number(dias) < -1) {
                    ...
                    switch (qtdeDiasUteis) {
                        case 2:
                        case 3:
                            log_1.default.audit('Aviso reativo ', '2/3 dias');
    ...
    function faturaEmail(faturaId) {
        var lista = []
        var busca = search_1.default.create({
            type: 'invoice',
            filters: [
                ['internalid' , 'IS', faturaId],
                "AND",
                ['custbody_o2s_transaction_l_tip_doc_fis', 'IS', 1, 2],
                "AND", 
                ["mainline","is","T"]
            ],
    ...
    var EmailPreventivo = function (data_de_vencimento, valor, dadosEmail) {
        var notaFiscal = faturaEmail(dadosEmail.faturaId)
        log.emergency("id da fatura linha 296", dadosEmail.faturaId)
        log.emergency("notaFiscal linha 296", notaFiscal)
    ...
    var EmailReativo = function (data_de_vencimento, valor, dadosEmail) {
        var notaFiscal = faturaEmail(dadosEmail.faturaId)
        log.emergency("id da fatura linha 306", dadosEmail.faturaId)
        log.emergency("notaFiscal linha 306", notaFiscal)
